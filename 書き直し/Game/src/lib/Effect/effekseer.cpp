...
//		エフェクト
//---------------------------------
void	CEffekseerCtrl::Load()
{
	LoadData("data/Effect/Windo.efk");
	LoadData("data/Effect/Windo2.efk");
	LoadData("data/Effect/Attack.efk");

	LoadData("data/Effect/hit.efk");
...
CEffekseerCtrl::~CEffekseerCtrl()
{
	OutputDebugStringA("CEffekseerCtrl Destructor\n");
	Exit();
}
...
{
	//
	OutputDebugStringA("CEffekseerCtrl Exit\n");	// ちょっと不安な破棄作業達
	if (m_eff != nullptr)
	{
		delete[]m_eff;
		m_eff = nullptr;
...
int	CEffekseerCtrl::Request(int ID, VECTOR pos, bool isLoop)
{
	...
	for (int i = 0; i < m_allNum; i++)
	{
		...
		{
			int hndl = m_manager->Play(m_effect[ID], pos.x, pos.y, pos.z);

			printfDx("Handle=%d\n", hndl);

			m_eff[number].SetHndl(hndl);
